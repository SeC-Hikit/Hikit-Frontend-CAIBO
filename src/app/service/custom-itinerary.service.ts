...
export class CustomItineraryService {
    ...
    downloadItinerary(customItinerary: CustomItineraryResult): Observable<any> {
        return this.httpClient.post<HttpResponse<Blob>>(
            this.baseUrl + "/itinerary-download", customItinerary, {
                responseType: "blob" as "json", observe: "response",
            }
        )
            .pipe(
                tap(),
                catchError(this.handleError<HttpResponse<Blob>>('Download a custom itinerary', null))
            );
    }
